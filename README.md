...
```bash
heartland-taxservice$ sam build
```
...
```bash
heartland-taxservice$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
heartland-taxservice$ sam local start-api
heartland-taxservice$ curl http://localhost:3000/
```
...
```bash
heartland-taxservice$ sam logs -n HelloWorldFunction --stack-name heartland-taxservice --tail
```
...
```bash
heartland-taxservice$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name heartland-taxservice
```
